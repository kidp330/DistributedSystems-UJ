...
// 	pxa[0].Start(6, "66")
// 	pxa[1].Start(7, "77")
// 	waitn(t, pxa, 0, npaxos) // Min() correct?
// 	for i := 0; i < npaxos; i++ {
// 		m := pxa[i].Min()
// 		if m != 0 {
...
// 		}
// 	}
// 	waitn(t, pxa, 1, npaxos) // Min() correct?
// 	for i := 0; i < npaxos; i++ {
// 		m := pxa[i].Min()
// 		if m != 0 {
// 			t.Fatalf("wrong Min() %v; expected 0", m)
...
// 	runtime.ReadMemStats(&m0) // m0.Alloc about a megabyte
// 	for i := 1; i <= 10; i++ {
...
// 	runtime.ReadMemStats(&m1) // m1.Alloc about 90 megabytes
// 	for i := 0; i < npaxos; i++ {
// 		pxa[i].Done(10)
// 	}
// 	for i := 0; i < npaxos; i++ {
...
// 	runtime.ReadMemStats(&m2) // m2.Alloc about 10 megabytes
...
// func DoNotTestRPCCount(t *testing.T) {
// 	runtime.GOMAXPROCS(4)
...
// 	if total1 > expected1 {
// 		t.Fatalf("too many RPCs for serial Start()s; %v instances, got %v, expected %v", ninst1, total1, expected1)
// 	}
// 	ninst2 := 5
// 	for i := 0; i < ninst2; i++ {
// 		for j := 0; j < npaxos; j++ {
...
// 	if total2 > expected2 {
// 		t.Fatalf("too many RPCs for concurrent Start()s; %v instances, got %v, expected %v", ninst2, total2, expected2)
// 	}
// 	fmt.Printf(" ... Passed\n")
// }
...
// 	}
// 	const ninst = 50
// 	for seq := 1; seq < ninst; seq++ { // only 5 active instances, to limit the // number of file descriptors.
...
// 	}
// 	const ninst = 50
// 	for seq := 1; seq < ninst; seq++ { // only 3 active instances, to limit the // number of file descriptors.
...
// 				pj := port(tag, p[j])
// 				err := os.Link(pj, ij)
// 				if err != nil { // one reason this link can fail is if the // corresponding Paxos peer has prematurely quit and // deleted its socket file (e.g., called px.Kill()).
...
// 	pxa[0].Start(seq, 1000) // poke them pxa[4].Start(seq, 1004) part(t, tag, npaxos, []int{0, 1, 2, 3, 4}, []int{}, []int{}) waitn(t, pxa, seq, npaxos)
// 	fmt.Printf(" ... Passed\n")
...
// 	defer part(t, tag, npaxos, []int{}, []int{}, []int{})
// 	done := false // re-partition periodically
// 	ch1 := make(chan bool)
// 	go func() {
// 		defer func() { ch1 <- true }()
// 		for done == false {
...
// 		}
// 	}()
// 	seq := 0 // periodically start a new instance
// 	ch2 := make(chan bool)
// 	go func() {
// 		defer func() { ch2 <- true }()
// 		for !done {
...
// 	<-ch1
// 	<-ch2
// 	<-ch3 // repair, then check that all instances decided.
// 	for i := 0; i < npaxos; i++ {
// 		pxa[i].unreliable = false
// 	}
